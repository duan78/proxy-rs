...
const SSL_INITIAL_BUFFER_SIZE: usize = 2048;
const MAX_RECEIVE_BUFFER_SIZE: usize = 10 * 1024 * 1024; // 10MB max receive buffer
...
impl Proxy {
    ...
    pub fn as_json(&self) -> String {
        ...
        serde_json::to_string(&proxy_data)
            .unwrap_or_else(|e| {
    ...
    pub async fn recv(&mut self, size: usize) -> Option<Vec<u8>> {
        ...
        // Bounds checking to prevent memory exhaustion
        if size > MAX_RECEIVE_BUFFER_SIZE {
            self.log(
                "Requested receive size exceeds maximum limit",
                Some(stime.elapsed()),
    ...
    pub async fn recv_all(&mut self) -> Option<Vec<u8>> {
        let stime = Instant::now();
        let mut chunk = vec![0; DEFAULT_RECV_BUFFER_SIZE];
        let mut buf = Vec::with_capacity(DEFAULT_RECV_BUFFER_SIZE);
        loop {
            // Check buffer size limit to prevent memory exhaustion
            if buf.len() >= MAX_RECEIVE_BUFFER_SIZE {
                self.log(
                    "Receive buffer size limit exceeded, closing connection",
                    Some(stime.elapsed()),
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    log::error!("Timeout error while receiving data from proxy {}: {}", self.as_text(), e);
                    break;
                }
            }
        }
    ...
    pub async fn close(&mut self) {
        // Close TLS connection first if it exists
        if self.tls_stream.is_some() {
            self.close_tls().await;
        }
        // Then close TCP connection if it still exists
...
impl Proxy {
    ...
    pub async fn connect_ssl(&mut self, connect_payload: &[u8]) -> bool {
        ...
        let mut chunk = vec![0; SSL_INITIAL_BUFFER_SIZE];
        match timeout(
            Duration::from_secs(self.timeout as u64),
        ...
        let config = match TlsConnector::builder()
            .danger_accept_invalid_certs(!self.verify_ssl)
            .build() {
            Ok(config) => config,
            Err(e) => {
                self.log(&format!("SSL: Failed to create TLS connector: {}", e),
                        Some(stime.elapsed()),
                        Some("tls_config_error".to_string()));
                return false;
            }
        };
