...
use hyper::{header::LOCATION, Request};
...
pub struct ProviderTask {
    client: Client<HttpsConnector<HttpConnector>, Empty<bytes::Bytes>>,
    base: Provider,
}
...
impl ProviderTask {
    ...
    fn build_get_request(&self, uri: &str) -> Request<Empty<bytes::Bytes>> {
        Request::builder()
            .uri(uri)
    ...
    #[async_recursion]
    async fn get_html(&self, request: Request<Empty<bytes::Bytes>>) -> String {
        ...
        .await
        {
            ...
            if let Ok(body) = body.collect().await {
